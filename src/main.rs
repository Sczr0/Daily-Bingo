...
impl Solver {
    ...
    fn solve(&self) -> Vec<Grid> {
        ...
        self.backtrack(0, 0, &mut current_grid, &mut solutions);
        solutions
    }
        ...
        Grid(
            (0..5).map(|i| {
                (0..5).map(|j| Cell {
                    ...
                    checked: self.color_grid[i][j] == Color::Black,
                }).collect()
            }).collect()
        )
    }

    fn backtrack(&self, x: usize, y: usize, grid: &mut Grid, solutions: &mut Vec<Grid>) {
        if x == 5 {
            if grid.has_five_in_a_row() 
                && grid.check_all_rules() 
                && grid.check_total_checked(self.max_checked) 
            {
                // 检查是否重复
                if !solutions.iter().any(|s| s.0 == grid.0) {
                    info!("🎉 找到有效解！总勾选数: {}",
                        grid.0.iter().flatten().filter(|c| c.checked).count()
                    );
        ...
        if self.color_grid[x][y] == Color::Black {
            ...
            self.backtrack(next_x, next_y, grid, solutions);
        } else {
            grid.0[x][y].checked = true;
            self.backtrack(next_x, next_y, grid, solutions);
        ...
        for (j, cell) in row.iter().enumerate() {
            ...
            if show_checks && cell.checked {
                ...
                draw_line_segment_mut(
                    &mut img,
                    (x_start as f32 + 5.0, y_start as f32 + 5.0),
                    ((x_start + cell_size - 5) as f32, (y_start + cell_size - 5) as f32),
                    Rgb([255u8, 0, 0]),
                );
                draw_line_segment_mut(
                    &mut img,
                    (x_start as f32 + 5.0, (y_start + cell_size - 5) as f32),
                    ((x_start + cell_size - 5) as f32, y_start as f32 + 5.0),
                    Rgb([255u8, 0, 0]),
                );
            }
        }
